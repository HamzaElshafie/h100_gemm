...
/**
 * @brief Tiled Shared Memory GEMM for BF16 (accumulates in FP32).
 *
...
__global__ void gemm_tiled_shared_bf16(const __nv_bfloat16* __restrict__ A,
    const __nv_bfloat16* __restrict__ B,
    __nv_bfloat16* __restrict__ C,
    int M, int N, int K, float alpha, float beta) {
    // Allocate shared memory
    __shared__ __nv_bfloat16 sharedA[TILE_SIZE * TILE_SIZE];
    __shared__ __nv_bfloat16 sharedB[TILE_SIZE * TILE_SIZE];
    ...
    for (int t = 0; t < num_tiles; t++) {
        ...
        for (int i = 0; i < TILE_SIZE; i++) {
            cumulative_sum += __bfloat162float(sharedA[ty * TILE_SIZE + i]) * __bfloat162float(sharedB[i * TILE_SIZE + tx]);
        }
    ...
    C[ty * K + tx] = __float2bfloat16_rn(alpha * cumulative_sum + beta * __bfloat162float(C[ty * K + tx]));
}
