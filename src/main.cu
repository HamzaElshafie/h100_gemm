...
KernelConfig parseKernelConfig(const std::string& impl, int kernel_id) {
    if (impl == "ampere") { // Check kernel validity
        if (kernel_id > 5 || kernel_id < 0) { // (TODO: Update later)
...
int main(int argc, char** argv) {
    ...
    if (config.type == KernelType::HOPPER) {
        ResourceManager<__nv_bfloat16> resources;
        resources.set_cublas_handle(&handle);
        resources.add_event(start);
        resources.add_event(stop);

        size_t mem_size = static_cast<size_t>(max_size) * static_cast<size_t>(max_size) * sizeof(__nv_bfloat16);

        // Allocate host memory (BF16)
        __nv_bfloat16* A_host = (__nv_bfloat16*)malloc(mem_size);
        __nv_bfloat16* B_host = (__nv_bfloat16*)malloc(mem_size);
        __nv_bfloat16* C_host = (__nv_bfloat16*)malloc(mem_size);
        __nv_bfloat16* C_host_ref = (__nv_bfloat16*)malloc(mem_size);
        ...
        // Initialise matrices (BF16)
        __nv_bfloat16* matrices[] = {A_host, B_host, C_host};
        initialiseArrays<__nv_bfloat16>(matrices, 3, static_cast<size_t>(max_size) * static_cast<size_t>(max_size), -100.0f, 100.0f, 0);

        // Allocate device memory (BF16)
        __nv_bfloat16* A_device;
        __nv_bfloat16* B_device;
        __nv_bfloat16* C_device;
        __nv_bfloat16* C_device_ref;
        ...
        for (int size: sizes) {
            ...
            size_t curr_mem_size = static_cast<size_t>(size) * static_cast<size_t>(size) * sizeof(__nv_bfloat16);

            std::cout << "Dimensions (M = N = K) = " << M << " Alpha: " << alpha << " Beta: " << beta << std::endl;

            // Copy data from host to device (BF16)
            ...
            // Run cuBLAS and custom kernel to check for correctness and warmup (BF16 baseline)
            if (config.type != KernelType::CUBLAS) {
                // Custom op (BF16)
                launchKernel<__nv_bfloat16>(config, A_device, B_device, C_device, M, N, K, alpha, beta, handle);
                CUDA_CHECK(cudaMemcpy(C_host, C_device, curr_mem_size, cudaMemcpyDeviceToHost));
                // cuBLAS op (BF16)
                KernelConfig cublas_config(KernelType::CUBLAS, 0);
                launchKernel<__nv_bfloat16>(cublas_config, A_device, B_device, C_device_ref, M, N, K, alpha, beta, handle);
                CUDA_CHECK(cudaMemcpy(C_host_ref, C_device_ref, curr_mem_size, cudaMemcpyDeviceToHost));
                // Verify results
                bool results_match = compareResults<__nv_bfloat16>(C_host_ref, C_host, static_cast<size_t>(M) * static_cast<size_t>(K), 2e-1f, 2e-1f);
                if (!results_match) {
            ...
            // Warmup cuBLAS kernel (BF16)
            KernelConfig cublas_config(KernelType::CUBLAS, 0);
            launchKernel<__nv_bfloat16>(cublas_config, A_device, B_device, C_device_ref, M, N, K, alpha, beta, handle);

            // Start cuBLAS timing (BF16)
            CUDA_CHECK(cudaEventRecord(start));
            // Run kernel multiple time to smooth out timing variations
            for (int i = 0; i < repeat; i++) {
                launchKernel<__nv_bfloat16>(cublas_config, A_device, B_device, C_device, M, N, K, alpha, beta, handle);
            }
            ...
            // Start custom kernel timing step (BF16)
            CUDA_CHECK(cudaEventRecord(start));
            for (int i = 0; i < repeat; i++) {
                launchKernel<__nv_bfloat16>(config, A_device, B_device, C_device, M, N, K, alpha, beta, handle);
            }
            ...
            // Copy result back to host (BF16)
            CUDA_CHECK(cudaMemcpy(C_host, C_device, curr_mem_size, cudaMemcpyDeviceToHost));
        }  // ResourceManager destructor will all handle cleanups

    } else {
        // Ampere Branch
        ResourceManager<float> resources; // RAII for float pointers, events, and cuBLAS handle
        resources.set_cublas_handle(&handle);
        resources.add_event(start);
        resources.add_event(stop);

        size_t mem_size = static_cast<size_t>(max_size) * static_cast<size_t>(max_size) * sizeof(float);
        ...
        initialiseArrays<float>(matrices, 3, static_cast<size_t>(max_size) * static_cast<size_t>(max_size), -100.0f, 100.0f, 0);
        ...
        for (int size: sizes) {
            ...
            // Calculate current memory size required
            size_t curr_mem_size = static_cast<size_t>(size) * static_cast<size_t>(size) * sizeof(float);
            ...
            if (config.type != KernelType::CUBLAS) {
                // Custom op
                launchKernel<float>(config, A_device, B_device, C_device, M, N, K, alpha, beta, handle);
                CUDA_CHECK(cudaMemcpy(C_host, C_device, curr_mem_size, cudaMemcpyDeviceToHost));
                // cuBLAS op
                KernelConfig cublas_config(KernelType::CUBLAS, 0);
                launchKernel<float>(cublas_config, A_device, B_device, C_device_ref, M, N, K, alpha, beta, handle);
                CUDA_CHECK(cudaMemcpy(C_host_ref, C_device_ref, curr_mem_size, cudaMemcpyDeviceToHost));
                // Verify results
                bool results_match = compareResults<float>(C_host_ref, C_host, static_cast<size_t>(M) * static_cast<size_t>(K), 2e-1f, 2e-1f);
                if (!results_match) {
                    std::cout << "Results do not match!" << std::endl;
                    return -1;
                } else {
            ...
            // Run kernel multiple time to smooth out timing variations
            for (int i = 0; i < repeat; i++) {
                launchKernel<float>(cublas_config, A_device, B_device, C_device, M, N, K, alpha, beta, handle);
            }
            CUDA_CHECK(cudaEventRecord(stop));
            CUDA_CHECK(cudaEventSynchronize(stop));
            CUDA_CHECK(cudaEventElapsedTime(&elapsed_time_fp32, start, stop));

            elapsed_time_fp32 /= 1000.;
            double cublas_avg_time = elapsed_time_fp32 / repeat;
            double cublas_tflops = flops_per_run / (cublas_avg_time * 1e12);
            ...
            elapsed_time_fp32 /= 1000.; // Convert to seconds
            double average_time = elapsed_time_fp32 / repeat;
            // Throughput in TFLOPs/s
            double custom_tflops = flops_per_run / (average_time * 1e12);
